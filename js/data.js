// js/data.js — Default product catalog from Crispi menus
// This data seeds localStorage on first load

const PLACEHOLDER_IMG = 'data:image/svg+xml,' + encodeURIComponent(
    '<svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" viewBox="0 0 80 80">' +
    '<rect width="80" height="80" fill="#1c2a4a"/>' +
    '<text x="40" y="44" text-anchor="middle" fill="#a0a0b0" font-size="10" font-family="sans-serif">No image</text>' +
    '</svg>'
);

const CATEGORIES = [
    { id: 'chawarma', name: 'Chawarma' },
    { id: 'poulet', name: 'Poulet Croustillant' },
    { id: 'kefta-mix', name: 'Kefta & Mix' },
    { id: 'petit-dejeuner', name: 'Petit Dejeuner' },
    { id: 'boissons', name: 'Boissons' }
];

const DEFAULT_PRODUCTS = [
    // === CHAWARMA ===
    { id: 'cw-01', name: 'Sandwich Chawarma', price: 15, category: 'chawarma', image: 'img/products/cw-01.jpeg', position: 1 },
    { id: 'cw-02', name: 'Chawarma Pains', price: 20, category: 'chawarma', image: 'img/products/cw-02.jpeg', position: 2 },
    { id: 'cw-03', name: 'Chawarma Frites', price: 25, category: 'chawarma', image: 'img/products/cw-03.jpeg', position: 3 },
    { id: 'cw-04', name: 'Tacos Chawarma', price: 30, category: 'chawarma', image: 'img/products/cw-04.jpeg', position: 4 },
    { id: 'cw-05', name: 'Double Chawarma', price: 35, category: 'chawarma', image: 'img/products/cw-05.jpeg', position: 5 },
    { id: 'cw-06', name: 'Chawarma Sarouki', price: 45, category: 'chawarma', image: 'img/products/cw-06.jpeg', position: 6 },
    { id: 'cw-07', name: 'Chawarma Box', price: 30, category: 'chawarma', image: 'img/products/cw-07.jpeg', position: 7 },
    { id: 'cw-08', name: 'Chawarma Big Box', price: 45, category: 'chawarma', image: 'img/products/cw-08.jpeg', position: 8 },
    { id: 'cw-09', name: 'Chawarma Arabe', price: 35, category: 'chawarma', image: 'img/products/cw-09.jpeg', position: 9 },
    { id: 'cw-10', name: 'Plat Chawarma', price: 40, category: 'chawarma', image: 'img/products/cw-10.jpeg', position: 10 },
    { id: 'cw-11', name: 'Kebsa Chawarma', price: 40, category: 'chawarma', image: 'img/products/Kebsa chawarma.PNG', position: 11 },
    { id: 'cw-fr', name: 'Frites', price: 5, category: 'chawarma', image: 'img/products/bs-08.jpeg', position: 12 },
    { id: 'cw-eau', name: 'Eau', price: 6, category: 'chawarma', image: 'img/products/bs-06.jpg', position: 13 },
    { id: 'cw-sem', name: 'Semonade', price: 5, category: 'chawarma', image: 'img/products/bs-07.jpeg', position: 14 },

    // === KEFTA & MIX ===
    { id: 'km-01', name: 'Sandwich Kefta', price: 30, category: 'kefta-mix', image: 'img/products/Sandwich kefta.PNG', position: 1 },
    { id: 'km-02', name: 'Tacos Kefta', price: 30, category: 'kefta-mix', image: 'img/products/Tacos kefta.PNG', position: 2 },
    { id: 'km-03', name: 'Sandwich Mix', price: 30, category: 'kefta-mix', image: 'img/products/Sandwich mix.PNG', position: 3 },
    { id: 'km-04', name: 'Tacos Mix', price: 30, category: 'kefta-mix', image: 'img/products/Tacos mix.PNG', position: 4 },
    { id: 'km-05', name: 'Cheese Burger', price: 30, category: 'kefta-mix', image: 'img/products/Cheese burger.PNG', position: 5 },
    { id: 'km-fr', name: 'Frites', price: 5, category: 'kefta-mix', image: 'img/products/bs-08.jpeg', position: 6 },
    { id: 'km-eau', name: 'Eau', price: 6, category: 'kefta-mix', image: 'img/products/bs-06.jpg', position: 7 },
    { id: 'km-sem', name: 'Semonade', price: 5, category: 'kefta-mix', image: 'img/products/bs-07.jpeg', position: 8 },

    // === POULET CROUSTILLANT ===
    { id: 'pk-02', name: 'Chicken Sandwich (sans frites)', price: 15, category: 'poulet', image: 'img/products/pk-02.jpeg', position: 1 },
    { id: 'pk-03', name: 'Chicken Sandwich', price: 20, category: 'poulet', image: 'img/products/pk-03.jpeg', position: 2 },
    { id: 'pk-04', name: 'Chicken Special', price: 28, category: 'poulet', image: 'img/products/pk-04.jpeg', position: 3 },
    { id: 'pk-05', name: 'Chicken Tacos', price: 25, category: 'poulet', image: 'img/products/pk-05.jpeg', position: 4 },
    { id: 'pk-06', name: 'Chicken Burger', price: 23, category: 'poulet', image: 'img/products/pk-06.jpeg', position: 5 },
    { id: 'pk-07', name: 'Tacos Royale', price: 45, category: 'poulet', image: 'img/products/pk-07.jpeg', position: 6 },
    { id: 'pk-08', name: 'Chicken Box', price: 30, category: 'poulet', image: 'img/products/pk-08.jpeg', position: 7 },
    { id: 'pk-09', name: 'Big Chicken Box', price: 45, category: 'poulet', image: 'img/products/pk-09.jpeg', position: 8 },
    { id: 'pk-10', name: 'Plat Chicken', price: 30, category: 'poulet', image: 'img/products/pk-10.jpeg', position: 9 },
    { id: 'pk-11', name: 'Kebesa Crispy', price: 40, category: 'poulet', image: 'img/products/Kebesa Crispy.PNG', position: 10 },
    { id: 'pk-fr', name: 'Frites', price: 5, category: 'poulet', image: 'img/products/bs-08.jpeg', position: 11 },
    { id: 'pk-eau', name: 'Eau', price: 6, category: 'poulet', image: 'img/products/bs-06.jpg', position: 12 },
    { id: 'pk-sem', name: 'Semonade', price: 5, category: 'poulet', image: 'img/products/bs-07.jpeg', position: 13 },

    // === PETIT DEJEUNER — Omelettes ===
    { id: 'om-01', name: 'Omelette Normal', price: 10, category: 'petit-dejeuner', image: 'img/products/om-01.jpg', position: 1 },
    { id: 'om-02', name: 'Omelette Fromage', price: 15, category: 'petit-dejeuner', image: 'img/products/om-02.jpg', position: 2 },
    { id: 'om-03', name: 'Omelette Khli3', price: 20, category: 'petit-dejeuner', image: 'img/products/om-03.jpg', position: 3 },
    { id: 'om-04', name: 'Omelette Avec Tomat', price: 20, category: 'petit-dejeuner', image: 'img/products/om-04.jpg', position: 4 },
    { id: 'om-05', name: 'Omelette Dinde Fumé', price: 20, category: 'petit-dejeuner', image: 'img/products/om-05.jpg', position: 5 },

    // === PETIT DEJEUNER — Ftour Complet ===
    { id: 'ff-01', name: 'Ftour Fassi Complet', price: 25, category: 'petit-dejeuner', image: 'img/products/ff-01.jpg', position: 6, description: 'Tamlita Tajin, Imen, Khobz, Atay' },
    { id: 'fc-01', name: 'Ftour Chamali Complet', price: 30, category: 'petit-dejeuner', image: 'img/products/fc-01.jpg', position: 7, description: 'Khli3 Alya, Jben, Fromage Hmer, Jm3ri, Zitoun, Khobz, Atay' },
    { id: 'fe-01', name: 'Ftour Express', price: 20, category: 'petit-dejeuner', image: PLACEHOLDER_IMG, position: 8, description: 'Harcha, Zebda, 3sel, K3mri, Atay' },
    { id: 'fb-01', name: 'Ftour Beldi Complet', price: 25, category: 'petit-dejeuner', image: 'img/products/fb-01.jpg', position: 9, description: 'Hmsa, Jahmriya, Khobza, Imen, Atay' },
    { id: 'pd-fr', name: 'Frites', price: 5, category: 'petit-dejeuner', image: 'img/products/bs-08.jpeg', position: 10 },
    { id: 'pd-eau', name: 'Eau', price: 6, category: 'petit-dejeuner', image: 'img/products/bs-06.jpg', position: 11 },
    { id: 'pd-sem', name: 'Semonade', price: 5, category: 'petit-dejeuner', image: 'img/products/bs-07.jpeg', position: 12 },

    // === BOISSONS ===
    { id: 'bs-01', name: 'Jus d\'Orange', price: 14, category: 'boissons', image: 'img/products/bs-01.jpg', position: 1 },
    { id: 'bs-02', name: 'Panache', price: 10, category: 'boissons', image: 'img/products/bs-02.jpg', position: 2 },
    { id: 'bs-03', name: 'Jus Avocat', price: 10, category: 'boissons', image: 'img/products/bs-03.jpg', position: 3 },
    { id: 'bs-04', name: 'Jus Banane', price: 10, category: 'boissons', image: 'img/products/bs-04.jpg', position: 4 },
    { id: 'bs-05', name: 'Jus Fraise', price: 10, category: 'boissons', image: 'img/products/bs-05.jpg', position: 5 },
    { id: 'bs-06', name: 'Eau', price: 6, category: 'boissons', image: 'img/products/bs-06.jpg', position: 6 },
    { id: 'bs-07', name: 'Semonade', price: 5, category: 'boissons', image: 'img/products/bs-07.jpeg', position: 7 },
    { id: 'bs-08', name: 'Frites', price: 5, category: 'boissons', image: 'img/products/bs-08.jpeg', position: 8 }
];
